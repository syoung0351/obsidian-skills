...
function rebuildSnapshotFromJournal(journalPath, id) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    try {
      ...
      diagnostics.push({ error: 'journal_parse_failed', line: i + 1, message: err.message });
    }
  }
  ...
  switch (event.type) {
    ...
    case 'agent_done':
      ...
      if (event.carbonize === true) {
        next.diagnostics.push({ error: 'carbonize_cleanup_required', file: event.file || null, message: 'Accepted variant still has carbonize markers that must be folded into source CSS.' });
      }
      break;
    case 'checkpoint':
